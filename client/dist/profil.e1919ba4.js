...
})({"dLawW":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "4670e7bce1919ba4";
"use strict";
...
},{}],"lLmDn":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
var _axios = require("./axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
(0, _axiosDefault.default).get("/user/getByIdUser").then((res)=>{
    if (res.data.message == "goLogin") return window.location.href = "/pages/login.html";
...
mediaInput.addEventListener("change", function() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            if (removImg) previewDiv.removeChild(removImg);
...
profilMedia.addEventListener("change", function() {
    ...
    if (file) {
        ...
        (0, _axiosDefault.default).put("/user/profilImg", formData, {
            headers: {
...
edit_profile_btn.addEventListener('click', (e)=>{
    if (boxBio.style.display === "none" || boxBio.style.display === "") boxBio.style.display = "block";
    else boxBio.style.display = "none";
});
formUserBio.addEventListener("submit", function(e) {
    e.preventDefault();
    const textarea = this.querySelector("textarea").value;
    if (!textarea.trim()) return alert("Enter Your Bio");
    boxBio.style.display = "none";
    document.getElementById("bio").textContent = textarea;
    const userBio = {
        ["bio"]: textarea
    };
    (0, _axiosDefault.default).put("/user/updateBio", userBio).then((res)=>console.log(res.data.message)).catch((err)=>console.log(err));
});
const formCreateCourse = document.querySelector(".formCreateCourse");
formCreateCourse.addEventListener('submit', function(e) {
    e.preventDefault();
    const formData = new FormData();
    if (!this.querySelector("#media").value) return alert("Please Insert a Picture");
    ...
    (0, _axiosDefault.default).post("/course/createCourse", formData, {
        headers: {
            "Content-Type": "multipart/form-data"
        }
    }).then((res)=>{
...
(0, _axiosDefault.default).get("/course/getByIdcategory").then((res)=>{
...
mediaInput2.addEventListener("change", function() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            if (existingVideo) previewDiv2.removeChild(existingVideo);
...
(0, _axiosDefault.default).get("/course/getCourseTitle").then((res)=>{
    if (res.data.message == "ok") createTitle(res.data.data);
    else console.log(res.data.message);
}).catch((err)=>console.log(err));
...
lessonForm.addEventListener('submit', function(e) {
    ...
    video.onloadedmetadata = function() {
        ...
        (0, _axiosDefault.default).post("/lesson/createLesson", formData).then((res)=>{
            if (res.data.message == 'The lesson number already exists') return alert("The lesson number already exists");
            else lessonForm.reset();
        }).catch((err)=>console.log(err));
    };
...
},{}]},["dLawW","lLmDn"], "lLmDn", "parcelRequiree8ef", {})

//# sourceMappingURL=profil.e1919ba4.js.map
